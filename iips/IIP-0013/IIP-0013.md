...
When the relevant feature flag is enabled (see [Activation](#activation)), the protocol builds `PackageMetadataV1` with the following dynamic-field layout:

1. A **metadata-layout version** marker under the `PackageMetadataVersionFieldName` key (starting from `2`, in order to distinguish from the implementation proposed for [IIP-0010](https://github.com/iotaledger/IIPs/blob/main/iips/IIP-0010/IIP-0010.md)). Its presence allows to understand which layout a developer must reference to.
